...
| 設定管理 | 「復元間隔」「除外アプリ」を `UserDefaults` に保存し、保存・復元処理に反映 |
...
設定は `UserDefaults`（標準ドメイン）に保存される。設定画面に出るのは次の2項目のみ。

| キー | 型 | 意味 | 既定値 |
|------|----|------|--------|
| `restoreDelay` | Int | 復元時、各ウィンドウ適用の間隔（ミリ秒。200〜5000 にクランプ） | 200 |
| `excludedApps` | [String] | 保存対象から除外するアプリ名（所有者名で一致） | なし |

> **設計方針**: 自動復元・ディスプレイ変化検知・クラウド同期などは「最小・ローカル完結」のコンセプトに照らして実装しない（[10. 制約事項](#10-制約事項) 参照）。
...
> 設定はファイルではなく `UserDefaults`（標準ドメイン）に保存される（[3.3 設定](#33-設定userdefaults) 参照）。
...
本アプリは「ウィンドウ配置を保存し、AX API で復元する」最小・ローカル完結のツールである。
以下はコンセプトに対して過剰なため、**実装しない**方針を明示する。

| 項目 | 不採用の理由 |
|------|------------|
| クラウド同期 | ローカル完結（外部通信なし）を維持するため |
| ウィンドウ配置の自動学習 | 手動の保存・復元で十分。複雑性に見合わない |
| プラグインシステム | 単機能ツールに拡張機構は不要 |
